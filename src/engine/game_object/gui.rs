use crate::engine;
use super::{GameObject, coordinate_transform::to_gl_space, gui_element, letter};
...
impl Gui {
    ...
    pub fn add_letter(&mut self, engine: &mut engine::Engine, letter: char, position: (f32, f32), size: (f32, f32)) {
        ...
        engine.add_object(letter, letter::Letter::new(&letter.to_string(), glm::vec3(pos_x, pos_y, 0.0), 
    ...
    pub fn add_background(&mut self, engine: &mut engine::Engine, element_name: &str, position: (f32, f32), size: (f32, f32)) {
        ...
        let size_x = (1.0 / (1000.0 / self.size.0) * (size.0 / self.size.0)) * 2.0;
        let size_y = (1.0 / (1000.0 / self.size.1) * (size.1 / self.size.1)) * 2.0;
        engine.add_object('e',gui_element::GuiElement::new(element_name, glm::vec3(pos_x, pos_y, 0.0), 
                                                            glm::vec3(size_x, size_y, 1.0), glm::vec3(0.0, 0.0, 0.0), 0.0));
    }
